...
# 
# Nix Environment: nix-all
# 
# If your platform supports it, using nix-shell is the simplest and most direct (and non-invasive)
# way to support holochain-rust development.  It avoids you finding/installing various Rust
# compilation toolchains and system libraries required in the correct versions required by
# holochain-rust.  However, it does require you to also use the same nix-shell to build your project
# targets (eg. to generate WASM from your Rust 'Zome code), because the Rust compiler only exists
# within the Nix environment created by the nix-shell.
# 
# Native Toolchain: all
# 
# If you prefer to use your native system toolchain, we have a set of targets that support most
# Debian/Ubuntu derived Linux systems, and Mac-OS.  However, specific versions of
#

# Native Toolchain {all,build,install,test} targets: Builds hc, holochain and WASM API, using natively
# installed Rust toolchain and libraries.
...
# NIX Environment nix-{all,build,install,test} targets: Builds hc, holochain and WASM API.  See
# `shell.nix` and the Nix environment manager `nix-shell`.  We don't want to contaminate the Nix
...
# There are 3 methods to obtain the native libsodium encryption library required by holochain-rust,
...
# rust_sodium-sys/build.rs.  The default is for rust_sodium-sys to download/compile libsodium 1.0.17
# (06-Jan-2019).  Uncomment out the option {0,1,2} RUST_ variables, as desired:
