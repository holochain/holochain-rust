...
module.exports = scenario => {
  ...
  scenario('get_nonexistent_entry', async (s, t) => {
    const { alice } = await s.players({ alice: one}, true)
    const getResult = await alice.call('app', 'blog', 'get_post', { post_address: 'QmY6MfiuhHnQ1kg7RwNZJNUQhwDxTFL45AAPnpJMNPEoxk' })
    t.deepEqual(getResult.Err, undefined) // no error returned
    t.deepEqual(getResult.Ok, null) // But no entry returned either
  })
