...
test('calling get_links before link_entries makes a difference', (t) => {
  ...
  const create1 = alice.call("blog", "create_post", {content: 'hi'})
  t.ok(create1.Ok)
  sleep.sleep(1)
...
test('alice create & publish post -> recommend own post to self', async (t) => {
  ...
  const numInitialRecommendedPosts = alice.call("blog", "my_recommended_posts", {}).Ok.addresses.length

  const postAddr = alice.call("blog", "create_post", params).Ok
  t.ok(postAddr)

  const gotPost = await pollFor(
    () => alice.call("blog", "get_post", {post_address: postAddr})
  ).catch(t.fail)
  t.ok(gotPost.Ok)

  let linked = alice.call("blog", "recommend_post", {
...
test('alice create & publish post -> tash recommend to self', async (t) => {
  ...
  // TODO can go away after scenario-api merge
  const numInitialRecommendedPosts = tash.call("blog", "my_recommended_posts", {}).Ok.addresses.length

  const postAddr = alice.call("blog", "create_post", params).Ok
  t.ok(postAddr)

  const gotPost = await pollFor(
    () => tash.call("blog", "get_post", {post_address: postAddr})
  ).catch(t.fail)
  t.ok(gotPost.Ok)

  let linked = tash.call("blog", "recommend_post", {
    post_address: postAddr,
...
test('create & publish post -> recommend to other agent', async (t) => {
  ...
  let linked = alice.call("blog", "my_recommended_posts", {
    post_address: postAddr,
    agent_address: tash.agentId
  })
  console.log("linked: ", linked)
