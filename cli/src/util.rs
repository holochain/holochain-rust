        ...
        Err(e) => {
            match e.kind() {
                ErrorKind::NotFound => {
                    ...
                    println!("Holochain requires you use the nightly-2019-02-22 toolchain.");
                    println!("With Rust already installed switch to it by running the following commands:");
                    println!("$ rustup toolchain install nightly-2019-02-22");
                    println!("$ rustup default nightly-2019-02-22");
                    match extra_help {
                        Some(messages) => {
