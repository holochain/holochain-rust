    ...
    pub fn load_config_with_signal(
        ...
        for ui_interface_config in config.ui_interfaces.clone() {
            ...
            let connected_dna_interface = 
                ui_interface_config.clone().dna_interface
                    .map(|interface_id| {
                        config.interface_by_id(&interface_id).unwrap()
                    });

            self.static_servers.insert(
                ui_interface_config.id.clone(),
                StaticServer::from_configs(ui_interface_config, bundle_config, connected_dna_interface),
            );
        }
