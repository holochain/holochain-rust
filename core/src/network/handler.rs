    ...
    Box::new(move |message| {
        ...
        match protocol_wrapper {
            Ok(ProtocolWrapper::StoreDht(dht_data)) => {
                ...
                let entry_with_header: EntryWithHeader = serde_json::from_str(&serde_json::to_string(&dht_data.content).unwrap()).unwrap();
                let maybe_address = block_on(hold_entry(&entry_with_header.entry.deserialize(), &context.clone()));
