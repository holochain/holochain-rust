use crate::{context::Context, dht::actions::{add_link::add_link, hold::hold_entry}, network::util::EntryWithHeader};
        ...
        match protocol_wrapper {
            ...
            Ok(ProtocolWrapper::StoreDhtMeta(dht_meta_data)) => {
                ...
                match dht_meta_data.attribute.as_ref() {
                    "link" => {
                        let link_add_entry = LinkAddEntry::from_entry(&entry_with_header.entry.deserialize());
                        let link = link_add_entry.link().clone();
                        let _ = block_on(add_link(
                            &link,
                            &context.clone(),
                        ));
                    },
