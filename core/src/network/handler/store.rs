    ...
    if let Ok(aspect) = aspect_json.clone().try_into() {
        match aspect {
            EntryAspect::Content(entry, header) => {
                ...
                thread::Builder::new().name(format!("store_entry_content/{}", ProcessUniqueId::new().to_string())).spawn(move || {
            ...
            EntryAspect::LinkAdd(link_data, header) => {
                ...
                let entry_with_header = EntryWithHeader { entry, header };
                thread::Builder::new().name(format!("store_link_entry/{}", ProcessUniqueId::new().to_string())).spawn(move || {
            ...
            EntryAspect::LinkRemove((link_data, links_to_remove), header) => {
                ...
                let entry_with_header = EntryWithHeader { entry, header };
                thread::Builder::new().name(format!("store_link_remove/{}", ProcessUniqueId::new().to_string())).spawn(move || {
                    if let Err(error) =
            ...
            EntryAspect::Update(entry, header) => {
                ...
                let entry_with_header = EntryWithHeader { entry, header };
                thread::Builder::new().name(format!("store_update/{}", ProcessUniqueId::new().to_string())).spawn(move || {
                    if let Err(error) =
            ...
            EntryAspect::Deletion(header) => {
                ...
                let entry_with_header = EntryWithHeader { entry, header };
                thread::Builder::new().name(format!("store_deletion/{}", ProcessUniqueId::new().to_string())).spawn(move || {
                    if let Err(error) =
