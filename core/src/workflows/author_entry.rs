...
pub async fn author_entry<'a>(entry: &'a Entry, context: &'a Arc<Context>) -> Result<Address, HolochainError> {
...
pub mod tests{
    ...
    fn test_commit_with_dht_publish() {
        ...
        let entry_address = block_on(
            author_entry(&test_entry(),&context1)
        );
        ...
        let json = state.dht().content_storage()
            .read()
            .unwrap()
            .fetch(&entry_address)
        ...
        assert_eq!(x,"{\"value\":\"\\\"test entry value\\\"\",\"entry_type\":\"testEntryType\"}".to_string());
    }
}
