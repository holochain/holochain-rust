...
fn hdk_commit(
    ...
    let output: CommitOutputStruct =
        try_deserialize_allocation(encoded_allocation_of_result as u32)?;
...
fn test_inner(mem_stack: &mut SinglePageStack) -> CommitOutputStruct {
    ...
    return if let Ok(hash_str) = hash {
        CommitOutputStruct { hash: hash_str }
    } else {
        CommitOutputStruct {
            hash: "fail".to_string(),
        }
...
fn hdk_commit_fail(mem_stack: &mut SinglePageStack) -> Result<String, HcApiReturnCode> {
    // Put args in struct and serialize into memory
