...
pub(crate) fn decode_pub_key(pub_key_b32: Base32, codec: &HcidEncoding) -> HcResult<SecBuf> {
    ...
    // convert to SecBuf
    let mut pub_key_sec = CRYPTO.buf_new_insecure(CRYPTO.sign_secret_key_bytes());
...
pub mod tests {
    ...
    fn it_should_verify_bufs() {
        ...
        // Create keys
        let mut public_key = TEST_CRYPTO.buf_new_insecure(TEST_CRYPTO.sign_secret_key_bytes());
