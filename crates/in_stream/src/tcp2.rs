...
#[derive(Debug)]
pub struct InStreamListenerTcp2(pub std::net::TcpListener);

impl InStreamListener2<&mut [u8], &[u8]> for InStreamListenerTcp2 {
    type Stream = InStreamTcp2;
    ...
    fn accept(&mut self) -> Result<<Self as InStreamListener2<&mut [u8], &[u8]>>::Stream> {
        let (stream, _addr) = self.0.accept()?;
        stream.set_nonblocking(true)?;
        InStreamTcp2::priv_new(stream, None)
    }
}

/// configuration options for tcp connect
pub struct TcpConnectConfig2 {
    pub connect_timeout_ms: Option<u64>,
}

impl Default for TcpConnectConfig2 {
    fn default() -> Self {
        Self {
...
impl InStream2<&mut [u8], &[u8]> for InStreamTcp2 {
    type ConnectConfig = TcpConnectConfig2;
...
impl InStream2Std<&mut [u8], &[u8]> for InStreamTcp2 {}
...
mod tests {
    ...
    fn tcp2_works() {
        ...
        let server_thread = std::thread::spawn(move || {
            let mut listener = InStreamListenerTcp2::bind(
        ...
        let client_thread = std::thread::spawn(move || {
            ...
            let mut cli = InStreamTcp2::connect(&binding, TcpConnectConfig2::default())
                .unwrap()
                .into_std_stream();
