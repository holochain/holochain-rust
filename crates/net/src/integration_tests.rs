...
pub mod tests {
        ...
        node::ipv4::machine(|ip| {
            ...
            loop {
                match sim2h.process() {
                    ...
                    Err(e) => println!("[server] error: {}", e.to_string()),
                }
                if false {
    ...
    fn client_machine(
        agent_id: AgentId,
        address_channel_rx: oneshot::Receiver<String>,
        handler: NetHandler,
    ) -> (impl node::ipv4::Ipv4Node, Sender<Lib3hClientProtocol>) {
        ...
        let machine = node::ipv4::machine(move |ip| {
            ...
            let client_config = Sim2hConfig {
                sim2h_url: sim2h_url.clone(),
            };
            println!(
                "[client] Client server at {} connecting to sim2h server at {}",
                ip,
                sim2h_url.clone()
            );
            ...
            loop {
                match worker.tick() {
                    Err(e) => println!(
                        "[client] Error occured in p2p network module, on tick: {:?}",
                        e
                    ),
                    Ok(_) => {
                        // println!("[client] tick")
    ...
    fn can_connect_to_server() {
        ...
        runtime
            .block_on(futures::future::lazy(move || {
                ...
                let (client_recipe, message_channel_tx) =
                    client_machine(test_agent, server_addr_rx, handler);
                ...
                let (spawn_complete, _ipv4_plug) =
                    network_handle.spawn_ipv4_tree(Ipv4Range::global(), router_recipe);
