...
fn main() {
    ...
    io.add_method("replace_conductor", move |params: Params| {
        if allow_replace_conductor {
            Ok(Value::String(os_eval(&format!("curl -L https://github.com/holochain/holochain-tracing/releases/download/{}/{} -o holochain.tar.gz && tar -xzvf holochain.tar.gz && mv holochain /holochain/.cargo/bin/holochain && rm holochain.tar.gz",
