...
The default interface is `websocket`. `http`can be simpler and faster for prototyping but `websocket` is recommended and for sure more adapted to the production usage. In fact with http it's only possible to pull data from the peers. With websocket both are possible, pull data from the peers and peers can push data to the users. This will be very useful in many cases, for instance to receive a confirmation when a certain data has been validated by the peers.
