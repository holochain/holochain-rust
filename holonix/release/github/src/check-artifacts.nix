let
  ...
  for binary in "''${core_binaries[@]}"; do for platform in "''${core_platforms[@]}"; do file="$binary-${release.core.tag}-x86_64-$platform.tar.gz"
  ...
  for node_version in "''${node_versions[@]}"; do for platform in "''${conductor_platforms[@]}"; do file="index-v${release.node-conductor.version.current}-node-v''${node_version}-''${platform}-x64.tar.gz"
