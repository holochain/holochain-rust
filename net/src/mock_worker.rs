...
impl MockSingleton {
    ...
    fn priv_handle_get_dht(&mut self, msg: &GetDhtData) -> NetResult<()> {
        match self.senders_by_dna.entry(msg.dna_address.clone()) {
            Entry::Occupied(mut e) => {
                if !e.get().is_empty() {
                    let r = &e.get_mut()[0];
...
mod tests {
    ...
    fn test_dna_address() -> Address {
        Address::from("blabladnahash")
    }

    fn test_agent_id_a() -> String {
        "agent-hash-test-1".into()
    }

    fn test_agent_id_b() -> String {
        "agent-hash_test-2".into()
    }
    ...
    fn it_mock_networker_flow() {
        ...
        cli1.receive(
            ProtocolWrapper::SendMessage(MessageData {
                dna_address: test_dna_address(),
                to_agent_id: test_agent_id_b(),
                from_agent_id: test_agent_id_a(),
