...
let
  ...
  if git tag | grep -q "${pulse.tag}"
   then
    ...
    echo "to push upstream run: git push ${github.config.upstream} ${pulse.tag}";
   else
    echo "tagging...";
    git tag -a ${pulse.tag} ${release.commit} -m 'Dev pulse ${pulse.version}';
    echo "pushing...";
    git push ${github.config.upstream} ${pulse.tag};
    echo $'pulse tag ${pulse.tag} created and pushed';
  fi
  echo
  echo 'pulse tag on github: https://github.com/${github.config.repo}/${github.config.repo-name}/releases/tag/${pulse.tag}'
  echo
  '';
in
script
