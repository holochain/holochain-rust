...
pub fn encrypt(message: &mut SecBuf,secret: &mut SecBuf,adata: &mut SecBuf, ad_len: libc::c_ulonglong,nonce: &mut SecBuf,cipher: &mut SecBuf)->SodiumResult<()>{
    unsafe {
        ...
        // let mut k = SecBuf::with_insecure(32);
        // let mut k = k.write_lock();
        let mut nonce = nonce.read_lock();
        let mut cipher = cipher.write_lock();
        let mut ci_len = cipher.len() as libc::c_ulonglong;
        let NULL: *mut u8 = std::ptr::null_mut();
        rust_sodium_sys::crypto_aead_xchacha20poly1305_ietf_encrypt(raw_ptr_char!(cipher),&mut ci_len,raw_ptr_char_immut!(message),_mess_len,raw_ptr_char_immut!(adata),ad_len,NULL,raw_ptr_char_immut!(nonce),raw_ptr_char_immut!(secret));
        Ok(())
    }
}
...
pub fn decrypt(decrypted_message: &mut SecBuf, secret: &mut SecBuf, adata: &mut SecBuf, ad_len: libc::c_ulonglong,nonce: &mut SecBuf, cipher: &mut SecBuf)->SodiumResult<()>{
    unsafe {
        ...
        let mut decrypted_message = decrypted_message.write_lock();
        let NULL: *mut u8 = std::ptr::null_mut();
        rust_sodium_sys::crypto_aead_xchacha20poly1305_ietf_decrypt(raw_ptr_char!(decrypted_message),std::ptr::null_mut(),NULL,raw_ptr_char_immut!(cipher),cipher_len,raw_ptr_char_immut!(adata),ad_len,raw_ptr_char_immut!(nonce),raw_ptr_char_immut!(secret));
        Ok(())
    }
}
