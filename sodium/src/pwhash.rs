...
pub fn hash(password: &mut SecBuf,ops_limit:u64,mem_limit:usize,alg:i8,salt:Option<&mut SecBuf>,hash:&mut SecBuf)->SodiumResult<()>{
    ...
    unsafe{
        rust_sodium_sys::crypto_pwhash(raw_ptr_char!(hash),hash_len,raw_ptr_ichar_immut!(password),pw_len,my_salt,ops_limit as libc::c_ulonglong,mem_limit,alg as libc::c_int);
        println!("> hash : {:?}",hash);
    }
    Ok(())
}
...
mod tests {
    ...
    fn it_should_generate_with_no_salt() {
        ...
        buf(&mut password);
        hash(&mut password,OPSLIMIT_SENSITIVE,MEMLIMIT_SENSITIVE,ALG_ARGON2ID13,None,&mut pw1_hash).unwrap();
    ...
    fn it_should_generate_with_salt() {
        ...
        // buf(&mut salt);
        hash(&mut password,OPSLIMIT_SENSITIVE,MEMLIMIT_SENSITIVE,ALG_ARGON2ID13,Some(&mut salt),&mut pw2_hash).unwrap();
    ...
    fn it_should_generate_consistantly() {
        ...
        let mut salt = SecBuf::with_insecure(SALTBYTES);
        buf(&mut salt);
        hash(&mut password,OPSLIMIT_SENSITIVE,MEMLIMIT_SENSITIVE,ALG_ARGON2ID13,Some(&mut salt),&mut pw1_hash).unwrap();
        hash(&mut password,OPSLIMIT_SENSITIVE,MEMLIMIT_SENSITIVE,ALG_ARGON2ID13,Some(&mut salt),&mut pw2_hash).unwrap();
