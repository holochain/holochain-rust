...
impl SinglePageAllocation {
    ...
    pub fn new(encoded_allocation: u32) -> Result<Self, HcApiReturnCode> {
        ...
        if (u32::from(allocation.offset) + u32::from(allocation.length))
...
impl SinglePageStack {
    ...
    pub fn new(last_allocation: SinglePageAllocation) -> Self {
        assert!(
            u32::from(last_allocation.offset) + u32::from(last_allocation.length)
                <= u32::from(u16::max_value())
        );
        SinglePageStack {
    ...
    pub fn allocate(&mut self, size: u16) -> u16 {
        assert!(u32::from(self.top) + u32::from(size) <= u32::from(u16::max_value()));
        let offset = self.top;
        self.top += size;
        offset
    }
...
pub unsafe fn deserialize<'s, T: Deserialize<'s>>(ptr_data: *mut c_char) -> T {
    let ptr_safe_c_str = CStr::from_ptr(ptr_data);
...
pub fn deserialize_allocation<'s, T: Deserialize<'s>>(encoded_allocation: u32) -> T {
    ...
    unsafe { deserialize(allocation.offset as *mut c_char) }
}
...
pub fn try_deserialize_allocation<'s, T: Deserialize<'s>>(
    ...
    Ok(unsafe { deserialize(allocation.offset as *mut c_char) })
}
...
pub fn serialize_into_encoded_allocation<T: Serialize>(
    ...
    allocation_of_output.encode() as i32
}
...
pub mod tests {
    ...
    fn new_spa() {
        let i = 0b1010_1010_1010_1010_0101_0101_0101_0101;
        let spa = SinglePageAllocation::new(i).unwrap();

        assert_eq!(0b1010_1010_1010_1010, spa.offset);

        assert_eq!(0b0101_0101_0101_0101, spa.length);
    }
    ...
    fn new_spa_error() {
        assert_eq!(
            // offset 0 = success?
            // @see https://github.com/holochain/holochain-rust/issues/181
            SinglePageAllocation::new(0b0000_0000_0000_0000_0000_0000_0000_0000).unwrap_err(),
            HcApiReturnCode::Success,
        );

        assert_eq!(
            // offset 1 = generic error
            SinglePageAllocation::new(0b0000_0000_0000_0001_0000_0000_0000_0000).unwrap_err(),
            HcApiReturnCode::Error,
        );

        assert_eq!(
            // offset 2 = serde json error
            SinglePageAllocation::new(0b0000_0000_0000_0010_0000_0000_0000_0000).unwrap_err(),
            HcApiReturnCode::ErrorJson,
        );

        assert_eq!(
            // offset 3 = page overflow error
            SinglePageAllocation::new(0b0000_0000_0000_0011_0000_0000_0000_0000).unwrap_err(),
            HcApiReturnCode::ErrorPageOverflow,
        );

        assert_eq!(
            // offset 4 = page overflow error
            SinglePageAllocation::new(0b0000_0000_0000_0100_0000_0000_0000_0000).unwrap_err(),
            HcApiReturnCode::ErrorActionResult,
        );

        assert_eq!(
            // nonsense offset = generic error
            SinglePageAllocation::new(0b1010_1010_1010_1010_0000_0000_0000_0000).unwrap_err(),
            HcApiReturnCode::Error,
        );
    }
    ...
    fn u32_high_bits() {
        assert_eq!(
            ...
            super::u32_high_bits(0b1010_1010_1010_1010_0101_0101_0101_0101),
        );
    }
    ...
    fn u32_low_bits() {
        assert_eq!(
            ...
            super::u32_low_bits(0b1010_1010_1010_1010_0101_0101_0101_0101),
        );
    }
    ...
    fn u32_split_bits() {
        assert_eq!(
            ...
            super::u32_split_bits(0b1010_1010_1010_1010_0101_0101_0101_0101),
        );
    }
    ...
    fn u32_merge_bits() {
        assert_eq!(
            ...
            super::u32_merge_bits(0b1010_1010_1010_1010, 0b0101_0101_0101_0101),
        );
    }
